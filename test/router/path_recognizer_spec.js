...
function main() {
    test_lib_1.describe('PathRecognizer', function () {
        ...
        test_lib_1.it('should return the same instruction instance when recognizing the same path', function () {
            ...
            test_lib_1.expect(firstMatch.instruction).toBe(secondMatch.instruction);
        });
        test_lib_1.describe('querystring params', function () {
            test_lib_1.it('should parse querystring params so long as the recognizer is a root', function () {
                ...
                test_lib_1.expect(match.instruction.params).toEqual({ 'name': 'igor' });
            });
            test_lib_1.it('should return a combined map of parameters with the param expected in the URL path', function () {
                ...
                var match = rec.recognize(url);
                test_lib_1.expect(match.instruction.params).toEqual({ 'name': 'paul', 'topic': 'success' });
            });
        });
        test_lib_1.describe('matrix params', function () {
            test_lib_1.it('should be parsed along with dynamic paths', function () {
                ...
                var match = rec.recognize(url);
                test_lib_1.expect(match.instruction.params).toEqual({ 'id': 'matias', 'key': 'value' });
            });
            test_lib_1.it('should be parsed on a static path', function () {
                ...
                var match = rec.recognize(url);
                test_lib_1.expect(match.instruction.params).toEqual({ 'name': 'dave' });
            });
            test_lib_1.it('should be ignored on a wildcard segment', function () {
                ...
                var match = rec.recognize(url);
                test_lib_1.expect(match.instruction.params).toEqual({ 'everything': 'super;variable=value' });
            });
            test_lib_1.it('should set matrix param values to true when no value is present', function () {
                ...
                var match = rec.recognize(url);
                test_lib_1.expect(match.instruction.params).toEqual({ 'one': true, 'two': true, 'three': '3' });
            });
            test_lib_1.it('should be parsed on the final segment of the path', function () {
                ...
                test_lib_1.expect(match.instruction.params).toEqual({ 'c': '3' });
            });
        });
    });
}
