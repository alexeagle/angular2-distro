...
function main() {
    test_lib_1.describe('MockViewResolver', function () {
        var viewResolver;
        test_lib_1.beforeEach(function () { viewResolver = new view_resolver_mock_1.MockViewResolver(); });
        test_lib_1.describe('View overriding', function () {
            test_lib_1.it('should fallback to the default ViewResolver when templates are not overridden', function () {
                var view = viewResolver.resolve(SomeComponent);
                test_lib_1.expect(view.template).toEqual('template');
                test_lib_1.expect(view.directives).toEqual([SomeDirective]);
            });
            test_lib_1.it('should allow overriding the @View', function () {
                viewResolver.setView(SomeComponent, new metadata_1.ViewMetadata({ template: 'overridden template' }));
                var view = viewResolver.resolve(SomeComponent);
                test_lib_1.expect(view.template).toEqual('overridden template');
                test_lib_1.expect(lang_2.isBlank(view.directives)).toBe(true);
            });
            test_lib_1.it('should not allow overriding a view after it has been resolved', function () {
        ...
        test_lib_1.describe('inline template definition overriding', function () {
            ...
            test_lib_1.it('should allow overriding an overriden @View', function () {
                viewResolver.setView(SomeComponent, new metadata_1.ViewMetadata({ template: 'overridden template' }));
                viewResolver.setInlineTemplate(SomeComponent, 'overridden template x 2');
                var view = viewResolver.resolve(SomeComponent);
                test_lib_1.expect(view.template).toEqual('overridden template x 2');
            });
            test_lib_1.it('should not allow overriding a view after it has been resolved', function () {
                viewResolver.resolve(SomeComponent);
                test_lib_1.expect(function () { viewResolver.setInlineTemplate(SomeComponent, 'overridden template'); })
                    .toThrowError("The component " + lang_1.stringify(SomeComponent) + " has already been compiled, its configuration can not be changed");
            });
        });
        test_lib_1.describe('Directive overriding', function () {
            test_lib_1.it('should allow overriding a directive from the default view', function () {
                viewResolver.overrideViewDirective(SomeComponent, SomeDirective, SomeOtherDirective);
                var view = viewResolver.resolve(SomeComponent);
                test_lib_1.expect(view.directives.length).toEqual(1);
                test_lib_1.expect(view.directives[0]).toBe(SomeOtherDirective);
            });
            test_lib_1.it('should allow overriding a directive from an overriden @View', function () {
                ...
                var view = viewResolver.resolve(SomeComponent);
                test_lib_1.expect(view.directives.length).toEqual(1);
                test_lib_1.expect(view.directives[0]).toBe(SomeComponent);
            });
            test_lib_1.it('should throw when the overridden directive is not present', function () {
                ...
                test_lib_1.expect(function () { viewResolver.resolve(SomeComponent); })
            ...
            test_lib_1.it('should not allow overriding a directive after its view has been resolved', function () {
                viewResolver.resolve(SomeComponent);
                test_lib_1.expect(function () {
                    viewResolver.overrideViewDirective(SomeComponent, SomeDirective, SomeOtherDirective);
                })
