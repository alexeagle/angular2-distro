...
function main() {
    test_lib_1.describe("DirectiveResolver", function () {
        var resolver;
        test_lib_1.beforeEach(function () { resolver = new directive_resolver_1.DirectiveResolver(); });
        test_lib_1.it('should read out the Directive metadata', function () {
            var directiveMetadata = resolver.resolve(SomeDirective);
            test_lib_1.expect(directiveMetadata)
                .toEqual(new metadata_1.DirectiveMetadata({ selector: 'someDirective', inputs: [], outputs: [], host: {}, queries: {} }));
        });
        test_lib_1.it('should throw if not matching metadata is found', function () {
            test_lib_1.expect(function () { resolver.resolve(SomeDirectiveWithoutMetadata); })
                .toThrowError('No Directive annotation found on SomeDirectiveWithoutMetadata');
        });
        test_lib_1.it('should not read parent class Directive metadata', function () {
        ...
        test_lib_1.describe('inputs', function () {
            test_lib_1.it('should append directive inputs', function () {
                var directiveMetadata = resolver.resolve(SomeDirectiveWithInputs);
                test_lib_1.expect(directiveMetadata.inputs).toEqual(['c', 'a', 'b: renamed']);
            });
            test_lib_1.it('should work with getters and setters', function () {
                var directiveMetadata = resolver.resolve(SomeDirectiveWithSetterProps);
                test_lib_1.expect(directiveMetadata.inputs).toEqual(['a: renamed']);
            });
            test_lib_1.it('should use properties as inputs', function () {
                var directiveMetadata = resolver.resolve(SomeDirectiveWithProperties);
                test_lib_1.expect(directiveMetadata.inputs).toEqual(['a']);
            });
        });
        test_lib_1.describe('outputs', function () {
            test_lib_1.it('should append directive outputs', function () {
                var directiveMetadata = resolver.resolve(SomeDirectiveWithOutputs);
                test_lib_1.expect(directiveMetadata.outputs).toEqual(['c', 'a', 'b: renamed']);
            });
            test_lib_1.it('should work with getters and setters', function () {
                var directiveMetadata = resolver.resolve(SomeDirectiveWithGetterOutputs);
                test_lib_1.expect(directiveMetadata.outputs).toEqual(['a: renamed']);
            });
            test_lib_1.it('should use events as outputs', function () {
                var directiveMetadata = resolver.resolve(SomeDirectiveWithEvents);
                test_lib_1.expect(directiveMetadata.outputs).toEqual(['a']);
            });
        });
        test_lib_1.describe('host', function () {
            test_lib_1.it('should append host bindings', function () {
                var directiveMetadata = resolver.resolve(SomeDirectiveWithHostBindings);
                test_lib_1.expect(directiveMetadata.host).toEqual({ '[c]': 'c', '[a]': 'a', '[renamed]': 'b' });
            });
            test_lib_1.it('should append host listeners', function () {
                var directiveMetadata = resolver.resolve(SomeDirectiveWithHostListeners);
                test_lib_1.expect(directiveMetadata.host)
                    .toEqual({ '(c)': 'onC()', '(a)': 'onA()', '(b)': 'onB($event.value)' });
            });
        });
        test_lib_1.describe('queries', function () {
            test_lib_1.it('should append ContentChildren', function () {
                var directiveMetadata = resolver.resolve(SomeDirectiveWithContentChildren);
                test_lib_1.expect(directiveMetadata.queries)
                    .toEqual({ "cs": new metadata_1.ContentChildren("c"), "as": new metadata_1.ContentChildren("a") });
            });
            test_lib_1.it('should append ViewChildren', function () {
            ...
            test_lib_1.it('should append ContentChild', function () {
            ...
            test_lib_1.it('should append ViewChild', function () {
