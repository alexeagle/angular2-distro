...
function main() {
    test_lib_1.describe("ViewResolver", function () {
        var resolver;
        test_lib_1.beforeEach(function () { resolver = new view_resolver_1.ViewResolver(); });
        test_lib_1.it('should read out the View metadata', function () {
            var viewMetadata = resolver.resolve(ComponentWithView);
            test_lib_1.expect(viewMetadata)
        ...
        test_lib_1.it('should read out the View metadata from the Component metadata', function () {
        ...
        test_lib_1.it('should read out the View metadata from a simple class', function () {
            var viewMetadata = resolver.resolve(ClassWithView);
            test_lib_1.expect(viewMetadata).toEqual(new metadata_1.View({ template: "some template" }));
        });
        test_lib_1.it('should throw when Component.template is specified together with the View metadata', function () {
            test_lib_1.expect(function () { return resolver.resolve(ComponentWithViewTemplate); })
                .toThrowErrorWith("Component 'ComponentWithViewTemplate' cannot have both 'template' and '@View' set at the same time");
        });
        test_lib_1.it('should throw when Component.template is specified together with the View metadata', function () {
            test_lib_1.expect(function () { return resolver.resolve(ComponentWithViewTemplateUrl); })
                .toThrowErrorWith("Component 'ComponentWithViewTemplateUrl' cannot have both 'templateUrl' and '@View' set at the same time");
        });
        test_lib_1.it('should throw when Component has no View decorator and no template is set', function () {
            test_lib_1.expect(function () { return resolver.resolve(ComponentWithoutView); })
                .toThrowErrorWith("Component 'ComponentWithoutView' must have either 'template', 'templateUrl', or '@View' set");
        });
        test_lib_1.it('should throw when simple class has no View decorator and no template is set', function () {
            test_lib_1.expect(function () { return resolver.resolve(SimpleClass); })
