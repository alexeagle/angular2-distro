...
function main() {
    test_lib_1.describe('ChangeDefinitionFactory', function () {
        ...
        var pipes;
        var eventLocals;
        test_lib_1.beforeEach(test_lib_1.inject([template_parser_1.TemplateParser], function (_templateParser) {
        ...
        function createChangeDetector(template, directives, protoViewIndex) {
            ...
            var protoChangeDetectors = change_definition_factory_1.createChangeDetectorDefinitions(new directive_metadata_1.CompileTypeMetadata({ name: 'SomeComp' }), change_detection_1.ChangeDetectionStrategy.Default, new change_detection_1.ChangeDetectorGenConfig(true, true, false, false), parser.parse(template, directives, 'TestComp'))
        ...
        test_lib_1.it('should watch element properties', function () {
            ...
            test_lib_1.expect(dispatcher.log).toEqual(['elementProperty(elProp)=someValue']);
        });
        test_lib_1.it('should watch text nodes', function () {
            var changeDetector = createChangeDetector('{{someProp}}', [], 0);
            context.someProp = 'someValue';
            changeDetector.detectChanges();
            test_lib_1.expect(dispatcher.log).toEqual(['textNode(null)=someValue']);
        });
        test_lib_1.it('should handle events', function () {
            ...
            test_lib_1.expect(context.eventLog).toEqual(['click']);
        });
        test_lib_1.it('should handle events with targets', function () {
        ...
        test_lib_1.it('should watch variables', function () {
            var changeDetector = createChangeDetector('<div #some-var [el-prop]="someVar">', [], 0);
            locals.set('someVar', 'someValue');
            changeDetector.detectChanges();
            test_lib_1.expect(dispatcher.log).toEqual(['elementProperty(elProp)=someValue']);
        });
        test_lib_1.it('should write directive properties', function () {
            ...
            context.someProp = 'someValue';
            changeDetector.detectChanges();
            test_lib_1.expect(directive.dirProp).toEqual('someValue');
        });
        test_lib_1.it('should write template directive properties', function () {
            var dirMeta = directive_metadata_1.CompileDirectiveMetadata.create({
                type: new directive_metadata_1.CompileTypeMetadata({ name: 'SomeDir' }),
                selector: '[dir-prop]',
                inputs: ['dirProp']
        ...
        test_lib_1.it('should watch directive host properties', function () {
            var dirMeta = directive_metadata_1.CompileDirectiveMetadata.create({
                type: new directive_metadata_1.CompileTypeMetadata({ name: 'SomeDir' }),
                selector: 'div',
                host: { '[elProp]': 'dirProp' }
        ...
        test_lib_1.it('should handle directive events', function () {
            var dirMeta = directive_metadata_1.CompileDirectiveMetadata.create({
                type: new directive_metadata_1.CompileTypeMetadata({ name: 'SomeDir' }),
                selector: 'div',
            ...
            eventLocals.set('$event', 'click');
            changeDetector.handleEvent('click', 0, eventLocals);
            test_lib_1.expect(directive.eventLog).toEqual(['click']);
        });
        test_lib_1.it('should create change detectors for embedded templates', function () {
            var changeDetector = createChangeDetector('<template>{{someProp}}<template>', [], 1);
            context.someProp = 'someValue';
            changeDetector.detectChanges();
            test_lib_1.expect(dispatcher.log).toEqual(['textNode(null)=someValue']);
        });
        test_lib_1.it('should watch expressions after embedded templates', function () {
