...
var selector_2 = require('angular2/src/core/compiler/selector');
function main() {
    test_lib_1.describe('SelectorMatcher', function () {
        ...
        test_lib_1.it('should select by element name case insensitive', function () {
            ...
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('SOMEOTHERTAG')[0], selectableCollector))
                .toEqual(false);
        ...
        test_lib_1.it('should select by class name case insensitive', function () {
            ...
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('.SOMEOTHERCLASS')[0], selectableCollector))
                .toEqual(false);
            test_lib_1.expect(matched).toEqual([]);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('.SOMECLASS')[0], selectableCollector)).toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1]);
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('.someClass.class2')[0], selectableCollector))
                .toEqual(true);
        ...
        test_lib_1.it('should select by attr name case insensitive independent of the value', function () {
            ...
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[SOMEOTHERATTR]')[0], selectableCollector))
                .toEqual(false);
            test_lib_1.expect(matched).toEqual([]);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[SOMEATTR]')[0], selectableCollector)).toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1]);
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[SOMEATTR=someValue]')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1]);
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[someAttr][someAttr2]')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1, s2[0], 2]);
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[someAttr=someValue][someAttr2]')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1, s2[0], 2]);
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[someAttr2][someAttr=someValue]')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1, s2[0], 2]);
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[someAttr2=someValue][someAttr]')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1, s2[0], 2]);
        });
        ...
        test_lib_1.it('should select by attr name and value case insensitive', function () {
            ...
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[SOMEATTR=SOMEOTHERATTR]')[0], selectableCollector))
                .toEqual(false);
            test_lib_1.expect(matched).toEqual([]);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[SOMEATTR=SOMEVALUE]')[0], selectableCollector))
                .toEqual(true);
        ...
        test_lib_1.it('should select by element name, class name and attribute name with value', function () {
            ...
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('someOtherTag.someOtherClass[someOtherAttr]')[0], selectableCollector))
                .toEqual(false);
            test_lib_1.expect(matched).toEqual([]);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('someTag.someOtherClass[someOtherAttr]')[0], selectableCollector))
                .toEqual(false);
            test_lib_1.expect(matched).toEqual([]);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('someTag.someClass[someOtherAttr]')[0], selectableCollector))
                .toEqual(false);
            test_lib_1.expect(matched).toEqual([]);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('someTag.someClass[someAttr]')[0], selectableCollector))
                .toEqual(false);
            test_lib_1.expect(matched).toEqual([]);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('someTag.someClass[someAttr=someValue]')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1]);
        });
        ...
        test_lib_1.it('should select independent of the order in the css selector', function () {
            ...
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('[someAttr].someClass')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1, s2[0], 2]);
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('.someClass[someAttr]')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1, s2[0], 2]);
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('.class1.class2')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s3[0], 3, s4[0], 4]);
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('.class2.class1')[0], selectableCollector))
                .toEqual(true);
        ...
        test_lib_1.it('should not select with a matching :not selector', function () {
            ...
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('p.someClass[someAttr]')[0], selectableCollector))
                .toEqual(false);
            test_lib_1.expect(matched).toEqual([]);
        });
        test_lib_1.it('should select with a non matching :not selector', function () {
            ...
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('p[someOtherAttr].someOtherClass')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1, s2[0], 2, s3[0], 3, s4[0], 4]);
        });
        test_lib_1.it('should match with multiple :not selectors', function () {
            matcher.addSelectables(s1 = selector_2.CssSelector.parse('div:not([a]):not([b])'), 1);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('div[a]')[0], selectableCollector)).toBe(false);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('div[b]')[0], selectableCollector)).toBe(false);
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('div[c]')[0], selectableCollector)).toBe(true);
        });
        test_lib_1.it('should select with one match in a list', function () {
            ...
            reset();
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('input[type=text]')[0], selectableCollector))
                .toEqual(true);
            test_lib_1.expect(matched).toEqual([s1[0], 1]);
        });
        test_lib_1.it('should not select twice with two matches in a list', function () {
            ...
            test_lib_1.expect(matcher.match(selector_2.CssSelector.parse('input.someclass')[0], selectableCollector))
                .toEqual(true);
    ...
    test_lib_1.describe('CssSelector.parse', function () {
        ...
        test_lib_1.it('should throw when nested :not', function () {
            test_lib_1.expect(function () { selector_2.CssSelector.parse('sometag:not(:not([attrname=attrvalue].someclass))')[0]; })
                .toThrowError('Nesting :not is not allowed in a selector');
        });
        test_lib_1.it('should throw when multiple selectors in :not', function () {
            test_lib_1.expect(function () { selector_2.CssSelector.parse('sometag:not(a,b)'); })
        ...
        test_lib_1.it('should detect lists of selectors with :not', function () {
            ...
            test_lib_1.expect(cssSelectors[1].notSelectors[0].element).toEqual('textarea');
            test_lib_1.expect(cssSelectors[2].element).toEqual('textbox');
            test_lib_1.expect(cssSelectors[2].notSelectors[0].classNames).toEqual(['special']);
        });
    });
    test_lib_1.describe('CssSelector.getMatchingElementTemplate', function () {
        test_lib_1.it('should create an element with a tagName, classes, and attributes', function () {
            ...
            test_lib_1.expect(template).toEqual('<blink class="neon hotpink" sweet dismissable="false"></blink>');
        });
        test_lib_1.it('should create an element without a tag name', function () {
            var selector = selector_2.CssSelector.parse('[fancy]')[0];
            var template = selector.getMatchingElementTemplate();
            test_lib_1.expect(template).toEqual('<div fancy></div>');
        });
        test_lib_1.it('should ignore :not selectors', function () {
            var selector = selector_2.CssSelector.parse('grape:not(.red)')[0];
            var template = selector.getMatchingElementTemplate();
            test_lib_1.expect(template).toEqual('<grape></grape>');
        });
    });
}
exports.main = main;
