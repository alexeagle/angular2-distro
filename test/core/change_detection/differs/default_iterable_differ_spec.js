...
function main() {
    test_lib_1.describe('iterable differ', function () {
        test_lib_1.describe('DefaultIterableDiffer', function () {
            var differ;
            test_lib_1.beforeEach(function () { differ = new default_iterable_differ_1.DefaultIterableDiffer(); });
            test_lib_1.it('should support list and iterables', function () {
                ...
                test_lib_1.expect(f.supports(new iterable_1.TestIterable())).toBeTruthy();
                test_lib_1.expect(f.supports(new Map())).toBeFalsy();
            ...
            test_lib_1.it('should support iterables', function () {
                ...
                differ.check(l);
                test_lib_1.expect(differ.toString()).toEqual(util_1.iterableChangesAsString({ collection: [] }));
                l.list = [1];
                differ.check(l);
            ...
            test_lib_1.describe('diff', function () {
                test_lib_1.it('should return self when there is a change', function () { test_lib_1.expect(differ.diff(['a', 'b'])).toBe(differ); });
                test_lib_1.it('should return null when there is no change', function () {
                    differ.diff(['a', 'b']);
                    test_lib_1.expect(differ.diff(['a', 'b'])).toEqual(null);
                });
