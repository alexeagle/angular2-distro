...
function main() {
     ...
     */
    test_lib_1.describe("MessageBus", function () {
        var bus;
        test_lib_1.beforeEach(function () { bus = message_bus_util_1.createConnectedMessageBus(); });
        test_lib_1.it("should pass messages in the same channel from sink to source", test_lib_1.inject([test_lib_1.AsyncTestCompleter], function (async) {
            ...
            async_1.ObservableWrapper.subscribe(fromEmitter, function (message) {
                test_lib_1.expect(message).toEqual(MESSAGE);
                async.done();
            });
            var toEmitter = bus.to(CHANNEL);
            async_1.ObservableWrapper.callNext(toEmitter, MESSAGE);
        }));
        test_lib_1.it("should broadcast", test_lib_1.inject([test_lib_1.AsyncTestCompleter], function (async) {
            var CHANNEL = "CHANNEL 1";
            var MESSAGE = "TESTING";
            var NUM_LISTENERS = 2;
            bus.initChannel(CHANNEL, false);
        ...
        test_lib_1.it("should keep channels independent", test_lib_1.inject([test_lib_1.AsyncTestCompleter], function (async) {
            ...
            async_1.ObservableWrapper.subscribe(firstFromEmitter, function (message) {
                test_lib_1.expect(message).toEqual(MESSAGE_ONE);
                callCount++;
                if (callCount == 2) {
                    async.done();
                }
            ...
            async_1.ObservableWrapper.subscribe(secondFromEmitter, function (message) {
                test_lib_1.expect(message).toEqual(MESSAGE_TWO);
                callCount++;
                if (callCount == 2) {
                    async.done();
                }
    ...
    test_lib_1.describe("PostMessageBusSink", function () {
        var bus;
        ...
        test_lib_1.beforeEach(function () { bus = message_bus_util_1.createConnectedMessageBus(); });
        test_lib_1.it("should buffer messages and wait for the zone to exit before sending", test_lib_1.inject([test_lib_1.AsyncTestCompleter, ng_zone_1.NgZone], function (async, zone) {
            setup(true, zone);
            var wasCalled = false;
            ...
            flushMessages(function () {
                test_lib_1.expect(wasCalled).toBeFalsy();
                zone.simulateZoneExit();
                flushMessages(function () {
                    test_lib_1.expect(wasCalled).toBeTruthy();
                    async.done();
                });
            });
        }), 500);
        test_lib_1.it("should send messages immediatly when run outside the zone", test_lib_1.inject([test_lib_1.AsyncTestCompleter, ng_zone_1.NgZone], function (async, zone) {
            setup(false, zone);
            var wasCalled = false;
            async_1.ObservableWrapper.subscribe(bus.from(CHANNEL), function (message) { wasCalled = true; });
            async_1.ObservableWrapper.callNext(bus.to(CHANNEL), "hi");
